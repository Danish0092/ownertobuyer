          ...
          <div className="grid grid-cols-2 gap-8 sm:grid-cols-3 lg:grid-cols-5">
            <div className="col-span-2 sm:col-span-3 lg:col-span-1">
              ...
              <p className="mb-4.5 max-w-[280px] text-[13px] leading-relaxed text-[#AEC2E0]">
                Lahore&apos;s owner-direct property marketplace. Buy Direct.
                Sell Free. Connect with property owners and dealers without
            ...
            />
            <FooterCol title="Company" links={["About Us", "Contact", "Help / FAQ"].map((label) => ({ label, href: "#" }))} />
            <FooterCol
              title="Legal"
              links={["Terms & Conditions", "Privacy Policy", "Safety / Disclaimer"].map((label) => ({ label, href: "#" }))}
            />
          </div>
          <div className="mt-10 border-t border-white/10 py-5 text-center text-xs text-[#8FA5C7]">
            © 2026 OwnerToBuyer. All rights reserved. A connection platform —
            not a real estate agency, broker or verification service.
          </div>
        </div>
      </footer>
    </div>
...
function FooterCol({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <div className="mb-3.5 font-display text-[13px] font-bold uppercase tracking-wide text-[#7DD3C0]">
        {title}
      </div>
